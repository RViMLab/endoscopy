...
def max_rectangle_in_circle(
    ...
    w = (
        ...
        * safety_radius
        / torch.sqrt(
            torch.tensor(
                [ratio**2 + 1], dtype=safety_radius.dtype, device=safety_radius.device
            )
        )
    )
    ...
    d0 = center
    d1 = (
        torch.tensor(list(img_shape[-2:]), device=center.device).expand_as(center)
        - center
    )
    ...
    shape = torch.where(
        ratio / new_ratio < 1,
        torch.stack(
            [ratio * new_ratio_shape[:, 1], new_ratio_shape[:, 1]], axis=-1
        ),  # update ratio preserving height
        torch.stack(
            [new_ratio_shape[:, 0], new_ratio_shape[:, 0] / ratio], axis=-1
        ),  # update ratio preserving width
    )
    ...
    return bbox_generator(
        top_left[:, 1], top_left[:, 0], shape[:, 1], shape[:, 0]
